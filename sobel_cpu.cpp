...
char output_fname[] = "./data/sobel_cpu_result.dat";
...
sobel_filtered_pixel(float *s, int i, int j , int ncols, int nrows, float *gx, float *gy)
{
      ...
      // The for loop is equivalent to the patten or i-ncols-1 .... seen above
      int k = 0;
      for (int m = -1; m<2; m++) {
         for (int n=-1; n<2; n++) {
            index = (i+(m*ncols)+n);
...
do_sobel_filtering(float *in, float *out, int ncols, int nrows)
{
   ...
   for (int c=1; c< ncols-1; c++) {
      for (int r=1; r<nrows-1; r++) {
         int i = (r*ncols) + c; 
         out[i] = sobel_filtered_pixel(in, i, 0, ncols, nrows, Gx, Gy);
      }
   }
}
